...
var wf_connColor = "#7e7e7f"; //连线的常规颜色
...
{
    var tables = [];
    $.ajax({ url: "GetTables?connid=" + connid, dataType: "json", async: false, success: function (json)
    {
...
{
    var fields = [];
    $.ajax({ url: "GetFields?connid=" + connid + "&table=" + table, dataType: "json", async: false, success: function (json)
    {
        for (var i = 0; i < json.length; i++)
        {
            fields.push(json[i]);
...
{
    var bbox = this.getBBox();
    var url = "/Platform/WorkFlow/Designer/Set_Step?appid=" + appid + "&id=" + this.id + "&x=" + bbox.x + "&y=" + bbox.y + "&width=" + bbox.width + "&height=" + bbox.height;
...
{
    var url = "/Platform/WorkFlow/Designer/Set_Line?appid=" + appid + "&id=" + this.id + "&from=" + this.fromid + "&to=" + this.toid;
...
{
    var url = "/Platform/WorkFlow/Designer/Set_Flow?appid=" + appid + "&isadd=" + (isAdd || 0).toString() + "&flowid=" + wf_json.id;
...
function saveFlow(op)
{
    ...
    var url = "/Platform/WorkFlow/Designer/Opation?appid=" + appid + "&flowid=" + wf_json.id + "&op=" + (op || "save");
...
function saveAs()
{
    ...
    var url = "/Platform/WorkFlow/Designer/SaveAs?appid=" + appid + "&flowid=" + wf_json.id;
