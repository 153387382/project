...
namespace NFine.Data.Extensions
{
    public class DbHelper
    {
        ...
        public static DataSet GetDataSet(string SQLString, SqlParameter[] cmdParms, int PageIndex, int PageSize, out int total)
        {
            using (SqlConnection connection = new SqlConnection(connstring))
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    try
                    {
                        ...
                        da.Fill(ds, firstResult, PageSize, "ds");
                        da.Fill(ds_total, "ds");
                        cmd.Parameters.Clear();
                        connection.Close();
                        connection.Dispose();
                        total = ds_total.Tables[0].Rows.Count;
                    }
        ...
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataTable GetTableGJ(string SQLString, params SqlParameter[] cmdParms)
        {
            using (SqlConnection connection = new SqlConnection(connstring))
            {
                ...
                try
                {
                    connection.Open();
                    SqlDataAdapter command = new SqlDataAdapter(SQLString, connection);
                    command.FillSchema(dt, SchemaType.Mapped);
                }
                catch (System.Data.SqlClient.SqlException ex)
                {
                    throw new Exception(ex.Message);
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    try
                    {
                        ...
                        foreach (DictionaryEntry myDE in SQLStringList)
                        {
                            ...
                            falg = Convert.ToBoolean(cmd.ExecuteNonQuery());
                            cmd.Parameters.Clear();
                        }
                        trans.Commit();
                    }
        ...
        /// <param name="tableName">DataSet结果中的表名</param>
        /// <returns>DataSet</returns>
        public static DataTable RunProc(string storedProcName, IDataParameter[] parameters)
        {
            using (SqlConnection connection = new SqlConnection(connstring))
            {
                DataTable dt = new DataTable();
